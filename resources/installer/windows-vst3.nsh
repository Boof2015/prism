...
; NOTE on parse order: this file is `!include`d BEFORE electron-builder's
; installer.nsi loads MUI2.nsh, so any reference to MUI macros (e.g.
; MUI_HEADER_TEXT) MUST live inside a macro body — macro bodies aren't parsed
; until `!insertmacro` runs them, which for customPageAfterChangeDir happens
; from inside assistedInstaller.nsh (post-MUI2). Functions therefore go inside
; the macro instead of at top level.
